% Grand-narrative presentation for the meeting.
%
% Compile with:  pdflatex grand_narrative.tex   (or tectonic)
%
% Beamer (default theme — drop into any base by removing \documentclass).
% 22 slides, ~30-minute meeting (~80 s per slide).
%
% Numbers on slide 12 (per-component ablation) are TARGETS, not measured.
% Frame in the meeting as "expected from the next vast.ai run, scheduled
% this week" — never as measured.
...
\begin{frame}{Paper 1 --- Multi-dataset DONUT training}
  ...
  \end{block}
  Repository: Donut-web-app.\quad Target venue: \textbf{IEEE Access}.
\end{frame}
...
\begin{frame}{Paper 2 --- Replication study + bug catalogue}
  ...
  \end{block}
  Repository: kaggle2 (\texttt{paper2/}).\quad Target venue: \textbf{IEEE Access}.
\end{frame}
...
\begin{frame}{Paper 3 --- SVKIE framework}
  ...
  \vspace{0.5em}
  Repository: kaggle2 (\texttt{paper3/}).\quad Target venue: \textbf{ICDAR main}.
\end{frame}
...
% --- Slide 12 ----------------------------------------------------------
\begin{frame}{Paper 3 --- Empirical contribution (target results)}
  6-cell ablation grid on canonical SROIE Task-3:
  \vspace{0.5em}
  \small
  \begin{tabular}{lc}
    \toprule
    Configuration & \textsf{total} F1 (target) \\ \midrule
  ...
  Numbers are the expected trajectory pending the next vast.ai run;
  single-seed baseline confirms 0.86.
\end{frame}
...
\begin{frame}{Paper 4 --- Compressed SVKIE (future work)}
  ...
  \end{itemize}
  Repository: future paper-4 repo.\quad Target venue: \textbf{efficient-ML workshop}
  (ES-FoMo / ENLSP).
\end{frame}
...
\begin{frame}{Reproducibility and open science}
  ...
  per-run \texttt{<paper-tree>/runs/<id>/} directory.
  \vspace{0.5em}
  \begin{itemize}
...
\begin{frame}{Roadmap and timeline}
  ...
  \begin{tabular}{ll}
    ...
    Paper 2 (lexical baseline) -- code complete           & Multi-seed run pending \\
    Paper 3 (SVKIE framework) -- code complete            & Multi-seed + ablation pending \\
