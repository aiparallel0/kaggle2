...
def write_pipeline_meta(config: ExpConfig) -> None:
    """Persist the live ``yolo_image_size`` so Bug 5 can be asserted later.

    Written at the end of training so :func:`report.combine.merge_pipeline_diagnostics`
    can compare the persisted value against ``config.yolo_image_size`` and
    ...
    with open(os.path.join(config.output_dir, "pipeline_meta.json"), "w") as f:
        json.dump({"yolo_image_size": config.yolo_image_size}, f)


def warn_below_expected(metrics: Metrics, config: ExpConfig, arch: str) -> None:
    """Soft-warn when F1 is below ``config.f1_warn_threshold`` (not an error)."""
    if metrics.global_f1 < config.f1_warn_threshold:
        log.warning(
            "%s F1=%.4f below f1_warn_threshold=%.2f (not an error).",
            arch, metrics.global_f1, config.f1_warn_threshold,
        )
