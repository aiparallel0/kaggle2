...
def test_paper_variant_advanced_picks_advanced_template(tmp_path: Path) -> None:
    """paper_variant=advanced selects template_advanced.tex when present."""
    cfg_path = _write_min_config(tmp_path, paper_variant="advanced")
    os.environ.pop("KAGGLE2_PAPER_VARIANT", None)
    config = load_config(str(cfg_path))
    assert config.paper_template.endswith("template_advanced.tex"), (
        f"expected template_advanced.tex, got {config.paper_template}"
    )


def test_paper_variant_basic_picks_basic_template(tmp_path: Path) -> None:
    """paper_variant=basic selects template_basic.tex when present."""
    cfg_path = _write_min_config(tmp_path, paper_variant="basic")
    os.environ.pop("KAGGLE2_PAPER_VARIANT", None)
    config = load_config(str(cfg_path))
    assert config.paper_template.endswith("template_basic.tex"), (
        f"expected template_basic.tex, got {config.paper_template}"
    )
...
def test_paper_variant_env_override_wins(tmp_path: Path) -> None:
    ...
    cfg_path = _write_min_config(tmp_path, paper_variant="advanced")
    os.environ["KAGGLE2_PAPER_VARIANT"] = "basic"
    try:
        config = load_config(str(cfg_path))
        assert config.paper_template.endswith("template_basic.tex")
    finally:
