...
_TEMPLATE_TEX = Path(__file__).resolve().parents[2] / "report" / "template.tex"
_TEMPLATE_BASIC = Path(__file__).resolve().parents[2] / "report" / "template_baseline.tex"
_TEMPLATE_ADVANCED = Path(__file__).resolve().parents[2] / "report" / "template_focus.tex"
...
def test_figref_template_baseline_alone_does_not_trigger(tmp_path: Path) -> None:
    """Verbatim \\figref from template_baseline.tex must not cause figure_absent_markers."""
    ...
    findings = scan_paper(paper)
    assert "figure_absent_markers" not in findings, (
        f"False positive from template_baseline.tex \\figref macro: {findings}"
    )


def test_figref_template_focus_alone_does_not_trigger(tmp_path: Path) -> None:
    """Verbatim \\figref from template_focus.tex must not cause figure_absent_markers."""
    ...
    findings = scan_paper(paper)
    assert "figure_absent_markers" not in findings, (
        f"False positive from template_focus.tex \\figref macro: {findings}"
    )
