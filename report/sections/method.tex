...
\VAR{n_train_images} train\,/\,\VAR{n_val_images} validation.
\IfAdvanced{The advanced (canonical) variant additionally evaluates
on the \VAR{n_test_images} official Task-3 test images (downloaded
...
pool.}\IfRulebased{The basic variant draws an additional
\VAR{n_test_images}-image internal test split from the same 626-image
pool (so train + val + test sum to 626).}  The split is computed
...
\VAR{n_train_images} receipts the decoder overfits less than at lower
...
The fine-tuning set is small (\VAR{n_train_images} receipts) but highly repetitive:
