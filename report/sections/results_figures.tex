...
\iffigurefile{fig_training_curves.pdf}{%
  ...
  \caption{DONUT training and evaluation loss together with evaluation F1
    ...
    (\VAR{n_val_images} receipts) so its empirical loss has lower variance.  The
...
%  \caption{Per-field token-F1 by system on the \VAR{n_test_images}-image test split.
...
%    is reproduced similarly by DONUT and the pipeline\IfRulebased{; the rule-based
...
\iffigurefile{fig_per_field_confusion.pdf}{%
  ...
  \caption{Per-field token F1 by system (DONUT, left; Pipeline, right)
    on the \VAR{n_test_images}-image SROIE test split.  Numeric values are annotated
