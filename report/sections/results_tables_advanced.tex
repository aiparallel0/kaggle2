...
Section~V cites for the \textbf{advanced} (canonical-test) variant
...
\emph{347-image canonical SROIE Task-3 test set}; the GT-OCR
$+$\,regex baseline that appears in the basic variant is omitted by
design, because Task-3 ships KIE entities only (no GT box / OCR
streams to feed a rule-based extractor).  An empty cell renders as
a typed \texttt{\textbackslash MissingCell\{key\}} marker --- never
a silent em-dash --- and every unresolved key is enumerated in
\texttt{metrics/unresolved\_vars.json} for audit.

\begin{table}[t]
\centering
\caption{Headline per-field F1 (\%) on the canonical SROIE Task-3
         test set (347 images).  DONUT (end-to-end,
         $\approx$\,200\,M parameters) vs.\ the
         YOLO+TrOCR+Attention-Assigner pipeline
         ($\approx$\,65\,M$+$\,\VAR{assigner_params_k}\,K
         parameters).  The GT-OCR baseline column is intentionally
         absent --- see Section~\ref{sec:limitations}.}
...
\caption{Training-run summary: number of epochs, best-checkpoint
         ...
         (\VAR{donut_params_m}\,M\,/\,\VAR{pipeline_params_m}\,M
         $\approx$\,4$\times$, matching the
         $\sim\VAR{param_ratio_numeric}$ ratio cited in the abstract)
